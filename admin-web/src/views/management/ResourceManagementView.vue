...
<template>
  <DomainPageShell v-model:dialog="dialog" title="资源中心管理" description="维护资源文件、元数据、可见范围与关联内容" noun="资源" icon="▱" :result="result" :selected="selected" :keyword="keyword" :status="status" :loading="loading" :error="error" :can-write="canWrite" :can-publish="canPublish" @update:keyword="list.keyword.value = $event" @update:status="list.status.value = $event" @select="list.select" @page="list.load" @retry="list.load()" @create="create" @save="save" @publish="publish" @archive="archive">
    <template #kpis><div class="kpi-grid"><AdminKpiCard icon="▱" label="资源总数" :value="result.total" color="#ff4d1f" /><AdminKpiCard icon="✓" label="已发布" :value="result.items.filter((item) => item.status === 'published').length" color="#22b66c" /><AdminKpiCard icon="⇩" label="当前下载" :value="detail?.downloads ?? '—'" color="#7c4dff" /><AdminKpiCard icon="◉" label="当前浏览" :value="detail?.views ?? '—'" color="#3478f6" /></div></template>
