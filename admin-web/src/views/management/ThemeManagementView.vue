...
const fields = reactive({ subtitle: '', introduction: '', cover: '', icon: '', accent: '', recommended: false, recommendedCourseIds: [] as string[], relatedLabIds: [] as string[], relatedResourceIds: [] as string[] })
...
const create = async (value: { slug: string; title: string; summary: string }) => {
...
<template>
  <DomainPageShell v-model:dialog="dialog" title="学习主题管理" description="管理主题视觉、学习路径与阶段内容" noun="主题" icon="▤" :result="result" :selected="selected" :keyword="keyword" :status="status" :loading="loading" :error="error" :can-write="canWrite" :can-publish="canPublish" @update:keyword="list.keyword.value = $event" @update:status="list.status.value = $event" @select="list.select" @page="list.load" @retry="list.load()" @create="create" @save="save" @publish="publish" @archive="archive">
    <template #kpis><div class="kpi-grid"><AdminKpiCard icon="▤" label="主题总数" :value="result.total" color="#ff4d1f" /><AdminKpiCard icon="✓" label="已发布" :value="result.items.filter((item) => item.status === 'published').length" color="#22b66c" /><AdminKpiCard icon="◇" label="路径阶段" :value="stages.length" color="#7c4dff" /><AdminKpiCard icon="↗" label="推荐主题" :value="result.items.filter((item) => item.data.recommended).length" color="#3478f6" /></div></template>
