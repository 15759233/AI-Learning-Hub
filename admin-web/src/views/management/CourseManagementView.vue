...
<template>
  <DomainPageShell v-model:dialog="dialog" title="课程内容管理" description="维护课程信息、章节、课时与结构化内容块" noun="课程" icon="▣" :result="result" :selected="selected" :keyword="keyword" :status="status" :loading="loading" :error="error" :can-write="canWrite" :can-publish="canPublish" @update:keyword="list.keyword.value = $event" @update:status="list.status.value = $event" @select="list.select" @page="list.load" @retry="list.load()" @create="create" @save="save" @publish="publish" @archive="archive">
    <template #kpis><div class="kpi-grid"><AdminKpiCard icon="▣" label="课程总数" :value="result.total" color="#ff4d1f" /><AdminKpiCard icon="✓" label="已发布" :value="result.items.filter((item) => item.status === 'published').length" color="#22b66c" /><AdminKpiCard icon="▤" label="当前章节" :value="detail?.chapters.length || 0" color="#7c4dff" /><AdminKpiCard icon="◇" label="当前课时" :value="detail?.chapters.reduce((sum, item) => sum + item.lessons.length, 0) || 0" color="#3478f6" /></div></template>
