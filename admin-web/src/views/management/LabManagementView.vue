...
<template>
  <DomainPageShell v-model:dialog="dialog" title="实训项目管理" description="配置受控实训类型、步骤、动作、校验与工具环境" noun="实训" icon="⬡" :result="result" :selected="selected" :keyword="keyword" :status="status" :loading="loading" :error="error" :can-write="canWrite" :can-publish="canPublish" @update:keyword="list.keyword.value = $event" @update:status="list.status.value = $event" @select="list.select" @page="list.load" @retry="list.load()" @create="create" @save="save" @publish="publish" @archive="archive">
    <template #kpis><div class="kpi-grid"><AdminKpiCard icon="⬡" label="实训总数" :value="result.total" color="#3478f6" /><AdminKpiCard icon="✓" label="已发布" :value="result.items.filter((item) => item.status === 'published').length" color="#22b66c" /><AdminKpiCard icon="▤" label="当前步骤" :value="detail?.steps.length || 0" color="#ff4d1f" /><AdminKpiCard icon="▦" label="当前工具" :value="tools.length" color="#7c4dff" /></div></template>
