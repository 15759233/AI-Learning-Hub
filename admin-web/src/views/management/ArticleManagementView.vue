...
<template>
  <DomainPageShell v-model:dialog="dialog" title="AI 前沿管理" description="维护文章正文、作者、推荐位与定时发布" noun="文章" icon="article" :result="result" :selected="selected" :keyword="keyword" :status="status" :loading="loading" :error="error" :can-write="canWrite" :can-publish="canPublish" @update:keyword="list.keyword.value = $event" @update:status="list.status.value = $event" @select="list.select" @page="list.load" @retry="list.load()" @create="create" @save="save" @publish="publish" @archive="archive">
