...
const fields = reactive({ category: '', tags: '', author: '', cover: '', readMinutes: 0, blocks: '' })
...
const create = async (value: { slug: string; title: string; summary: string }) => { await drafts.createDraft({ ...value, category: '大模型' }); dialog.value = false; await list.load(1); ElMessage.success('文章草稿已创建') }
const save = async (base: { title: string; summary: string; sortOrder: number }) => { if (list.selected.value) { await drafts.saveDraft(list.selected.value, { ...base, ...input() }); await list.load(); ElMessage.success('文章正文草稿已保存') } }
...
<template>
  <DomainPageShell v-model:dialog="dialog" title="AI 前沿管理" description="维护文章正文、作者、推荐位与定时发布" noun="文章" icon="◉" :result="result" :selected="selected" :keyword="keyword" :status="status" :loading="loading" :error="error" :can-write="canWrite" :can-publish="canPublish" @update:keyword="list.keyword.value = $event" @update:status="list.status.value = $event" @select="list.select" @page="list.load" @retry="list.load()" @create="create" @save="save" @publish="publish" @archive="archive">
    <template #kpis><div class="kpi-grid"><AdminKpiCard icon="◉" label="文章总数" :value="result.total" color="#3478f6" /><AdminKpiCard icon="✓" label="已发布" :value="result.items.filter((item) => item.status === 'published').length" color="#22b66c" /><AdminKpiCard icon="◷" label="待发布" :value="result.items.filter((item) => item.status === 'reviewing').length" color="#f2a500" /><AdminKpiCard icon="↗" label="当前阅读" :value="detail?.views ?? '—'" color="#7c4dff" /></div></template>
    ...
    <template #editor>
      ...
      <section class="domain-section"><h3>文章内容</h3><label>分类<select v-model="fields.category"><option v-for="name in ['大模型','Agent','多模态','机器人','AI 安全']" :key="name">{{ name }}</option></select></label><label>标签（逗号分隔）<input v-model="fields.tags" /></label><label>作者<input v-model="fields.author" /></label><label>封面<input v-model="fields.cover" /></label><label>预计阅读分钟<input v-model.number="fields.readMinutes" type="number" min="0" /></label><label>正文段落（空行分段）<textarea v-model="fields.blocks" rows="8" /></label></section>
