...
<template>
  <DomainPageShell v-model:dialog="dialog" title="挑战测评管理" description="管理挑战、题库、试卷、知识点与题型草稿" noun="挑战" icon="▦" :result="result" :selected="selected" :keyword="keyword" :status="status" :loading="loading" :error="error" :can-write="canWrite" :can-publish="canPublish" @update:keyword="list.keyword.value = $event" @update:status="list.status.value = $event" @select="list.select" @page="list.load" @retry="list.load()" @create="create" @save="save" @publish="publish" @archive="archive">
    <template #kpis><div class="kpi-grid"><AdminKpiCard icon="▦" label="挑战总数" :value="result.total" color="#ff4d1f" /><AdminKpiCard icon="▤" label="题库" :value="banks.length" color="#3478f6" /><AdminKpiCard icon="◇" label="当前题目" :value="questions.length" color="#7c4dff" /><AdminKpiCard icon="✓" label="已发布挑战" :value="result.items.filter((item) => item.status === 'published').length" color="#22b66c" /></div></template>
