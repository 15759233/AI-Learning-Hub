...
const summary = ref<CommunityAdminSummaryDto | null>(null), posts = ref<CommunityPostDetailDto[]>([]), comments = ref<AdminCommunityComment[]>([]), topics = ref<CommunityTopicDto[]>([]), reports = ref<CommunityAdminReportDto[]>([]), officials = ref<Array<CommunityAuthorDto & { expertiseTopics: string[] }>>([]), policy = ref<CommunityFeedPolicyDto | null>(null)
...
const load = async () => {
  ...
  try {
    summary.value = await communityAdminApi.summary()
    if (['posts', 'questions'].includes(tab.value)) posts.value = await communityAdminApi.posts(tab.value === 'questions', keyword.value)
    if (tab.value === 'comments') comments.value = await communityAdminApi.comments()
    if (tab.value === 'topics') topics.value = await communityAdminApi.topics()
    if (tab.value === 'reports') reports.value = await communityAdminApi.reports()
    if (tab.value === 'official') officials.value = await communityAdminApi.officials()
    if (tab.value === 'policy') policy.value = await communityAdminApi.policy()
  } catch (cause) { error.value = cause instanceof Error ? cause.message : '社区运营数据读取失败' } finally { loading.value = false }
}
const inspect = async (id: string) => { try { selected.value = await communityAdminApi.inspection(id) } catch (cause) { error.value = cause instanceof Error ? cause.message : '读取详情失败' } }
const total = computed(() => ['posts', 'questions'].includes(tab.value) ? posts.value.length : tab.value === 'comments' ? comments.value.length : tab.value === 'topics' ? topics.value.length : tab.value === 'reports' ? reports.value.length : officials.value.length)
const slice = <T,>(items: T[]) => items.slice((page.value - 1) * 20, page.value * 20)
...
const officialOpen = ref(false), officialId = ref(''), officialForm = reactive({ verifiedType: 'none', expertise: '', reason: '' })
const editOfficial = (user: CommunityAuthorDto & { expertiseTopics: string[] }) => { officialId.value = user.id; Object.assign(officialForm, { verifiedType: user.verifiedType, expertise: user.expertiseTopics.join(','), reason: '' }); officialOpen.value = true }
const saveOfficial = async () => { try { await communityAdminApi.verify(officialId.value, officialForm.verifiedType, officialForm.expertise.split(',').map((t) => t.trim()).filter(Boolean), officialForm.reason); officialOpen.value = false; await load() } catch (cause) { error.value = cause instanceof Error ? cause.message : '认证保存失败' } }
const policyForm = reactive({ parameter: 'learningWeight', value: 28, reason: '' })
const savePolicy = async () => { try { await communityAdminApi.updatePolicy(policyForm.parameter, policyForm.value, policyForm.reason); policyForm.reason = ''; await load() } catch (cause) { error.value = cause instanceof Error ? cause.message : '策略保存失败' } }
...
const savePost = async () => {
  try {
    const original = editingPost.value ? selected.value?.post : null
    const input = { type: original?.type || 'general' as const, title: postForm.title, contentBlocks: [{ type: 'paragraph' as const, text: postForm.text }, ...(original?.contentBlocks.filter((b) => b.type !== 'paragraph') || [])], bindings: original?.bindings.filter((b) => b.status !== 'unavailable').map((b) => ({ type: b.type, id: b.id })) || [], topicIds: original?.topics.map((t) => t.id) || [], visibility: original?.visibility || 'public' as const, status: original?.status === 'draft' ? 'draft' as const : 'published' as const, reason: postForm.reason }
    if (editingPost.value) await communityAdminApi.editPost(postTarget.value, input); else await communityAdminApi.officialPost(postTarget.value, input)
...
watch(tab, () => { page.value = 1; selected.value = null; void load() })
onMounted(load)
</script>
<template><div class="community-admin-page">
  ...
  <section class="panel community-admin-workspace"><nav class="community-admin-tabs" aria-label="社区运营分类"><button v-for="item in tabs.filter((t) => can(t.permission))" :key="item.key" :class="{ active: tab === item.key }" @click="tab = item.key">{{ item.label }}</button></nav>
    <div v-if="['posts', 'questions'].includes(tab)" class="community-admin-split">
      <div class="community-admin-list"><form class="community-admin-filter" @submit.prevent="page = 1; load()"><input v-model="keyword" placeholder="搜索正文或学习问题" maxlength="120" /><button class="admin-secondary">查询</button></form><div class="community-admin-table"><table><thead><tr><th>作者 / 内容</th><th>学习关联</th><th>{{ tab === 'questions' ? '回答状态' : '互动' }}</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="post in slice(posts)" :key="post.id" :class="{ selected: selected?.post.id === post.id }"><td><small>{{ post.author.displayName }} · {{ typeLabels[post.type] }}</small><strong>{{ post.title || post.bodyPreview }}</strong><p>{{ post.bodyPreview }}</p></td><td><span v-for="binding in post.bindings.slice(0, 2)" :key="binding.id">{{ binding.title }}</span></td><td><template v-if="post.question">{{ post.question.status === 'solved' ? '已解决' : '待回答' }}<small>{{ post.stats.comments }} 回答 · {{ post.question.teacherAnswered ? '教师已参与' : '等待讨论' }}</small></template><template v-else>{{ post.stats.useful }} 有帮助<small>{{ post.stats.bookmarks }} 收藏 · {{ post.stats.comments }} 评论</small></template></td><td><span class="community-admin-status" :class="post.status">{{ statusLabels[post.status] }}</span><small>{{ new Date(post.publishedAt).toLocaleDateString('zh-CN') }}</small></td><td><button class="admin-text" @click="inspect(post.id)">详情</button></td></tr></tbody></table></div><p v-if="!posts.length && !loading" class="admin-empty">没有匹配的动态</p><AdminPagination :page="page" :page-size="20" :total="total" @change="page = $event" /></div>
    ...
    <div v-else-if="tab === 'comments'" class="community-admin-section"><article v-for="comment in slice(comments)" :key="comment.id" class="community-admin-row"><div><strong>{{ comment.author.displayName }}</strong><p>{{ comment.body }}</p><small>{{ statusLabels[comment.status] }} · {{ new Date(comment.createdAt).toLocaleString('zh-CN') }}</small></div><button v-if="can('community.moderate')" class="admin-secondary" @click="openModeration('comment', comment.id)">处理评论</button></article><AdminPagination :page="page" :page-size="20" :total="total" @change="page = $event" /></div>
    ...
    <div v-else-if="tab === 'reports'" class="community-admin-section"><article v-for="report in slice(reports)" :key="report.id" class="community-admin-row"><div><strong>{{ report.reason }}</strong><p>{{ report.description || '未补充说明' }}</p><small>{{ statusLabels[report.status] }} · {{ new Date(report.createdAt).toLocaleString('zh-CN') }}</small></div><button v-if="can('community.moderate')" class="admin-primary" @click="openModeration('report', report.id)">处理举报</button></article><p v-if="!reports.length" class="admin-empty">暂无待处理举报</p><AdminPagination :page="page" :page-size="20" :total="total" @change="page = $event" /></div>
    <div v-else-if="tab === 'official'" class="community-admin-section"><p class="community-admin-note">认证与发布沿用统一用户和 RBAC。认证字段由服务端校验，学生不能自行申领教师或官方身份。</p><article v-for="user in slice(officials)" :key="user.id" class="community-admin-row"><div><strong>{{ user.displayName }}</strong><p>{{ user.username }} · {{ user.school }}</p><small>{{ user.verifiedType === 'none' ? '普通学习者' : user.verifiedType }}</small></div><button class="admin-secondary" @click="editOfficial(user)">管理认证</button></article><AdminPagination :page="page" :page-size="20" :total="total" @change="page = $event" /></div>
  ...
  <AdminDialog v-model="postOpen" :title="editingPost ? '编辑社区内容' : '发布官方学习指导'"><form class="admin-form" @submit.prevent="savePost"><label>标题<input v-model="postForm.title" maxlength="160" required /></label><label>正文<textarea v-model="postForm.text" minlength="5" maxlength="20000" rows="8" required /></label><p v-if="editingPost">关联学习内容、代码、图片与引用块保持不变。</p><label>操作理由<textarea v-model="postForm.reason" minlength="4" maxlength="500" required /></label><button class="admin-primary">确认{{ editingPost ? '保存' : '发布' }}</button></form></AdminDialog>
