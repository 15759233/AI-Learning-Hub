...
<template>
  <button type="button" class="landing-content-card" :class="`landing-card-${variant}`" @click="$emit('open', item)">
    <img v-if="hasImage" class="landing-cover" :src="landingAsset(String(item.data.cover || ''), cover)" :alt="`${item.title}主题示意图`" width="960" height="540" :loading="variant === 'featured' ? 'lazy' : 'eager'" decoding="async" />
