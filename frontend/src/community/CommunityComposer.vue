...
onBeforeUnmount(() => { removeGuard(); window.removeEventListener('beforeunload', leave) })
</script>
<template>
...
<AppDialog v-model="editor.closePrompt" title="保留未完成的内容"><p>还有正在编辑的内容，你希望怎样离开？</p><div class="composer-actions"><button class="button primary" :disabled="editor.saving" @click="finish(true)">保存草稿</button><button class="button secondary" @click="finish(false)">放弃修改</button><button class="button secondary" @click="editor.closePrompt = false; continuation = null">继续编辑</button></div></AppDialog>
</template>
