...
const following = ref<CommunityAuthorDto[]>([])
let loadEpoch = 0
...
<template><section><header class="community-page-heading"><div><RouterLink to="/community">← 社区发现</RouterLink><h1>{{ title }}</h1><p v-if="topic">{{ topic.description }}</p><p v-if="profile">{{ profile.headline || '在学习与实践中，一起向前。' }}</p></div><button v-if="topic || (profile && profile.id !== auth.user?.id)" class="button primary small" @click="follow">{{ (topic?.following || profile?.following) ? '已关注' : '关注' }}</button></header>
  ...
  <p v-if="error" class="community-error" role="alert">{{ error }} <button @click="load">重试</button></p><p v-if="loading" class="community-empty" role="status">正在读取内容…</p>
  <div v-else-if="tab === 'following'" class="community-collection"><RouterLink v-for="person in following" :key="person.id" class="community-binding" :to="`/community/user/${person.username}`">{{ person.displayName }} · {{ badgeLabels[person.verifiedType] }}</RouterLink><p v-if="!following.length">还没有关注其他学习者。</p></div>
  <div v-else-if="tab === 'topics'" class="community-collection"><RouterLink v-for="item in profile?.topics" :key="item.id" class="community-binding" :to="`/community/topic/${item.slug}`"># {{ item.name }} · {{ item.postCount }} 条讨论</RouterLink><p v-if="!profile?.topics.length">还没有关注学习话题。</p></div>
  ...
  <template v-else><CommunityPostCard v-for="post in posts" :key="post.id" :post="post" @changed="load" @hidden="load" /><div v-if="!posts.length && !error" class="community-empty"><h2>这里还没有内容</h2><p>分享一个发现，或从社区首页开始探索。</p></div></template>
</section></template>
