...
const act = async (action: () => Promise<unknown>, refresh = true) => { if (!requireWrite()) return; pending.value = true; error.value = ''; try { await action(); if (refresh) emit('changed') } catch (cause) { error.value = cause instanceof Error ? cause.message : '操作失败' } finally { pending.value = false } }
...
const pin = () => { if (requireWrite()) emit('pin', props.pinned ? null : props.post.id) }
...
const unpublish = () => act(async () => { await communityApi.unpublish(props.post.id); unpublishOpen.value = false; store.removePost(props.post.id); emit('hidden', props.post.id) }, false)
