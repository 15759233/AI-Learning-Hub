...
import type { CommunityPostSummaryDto, CommunityBindingDto, CommunitySignalInput, CommunityExportFormat } from '@ai-learning-hub/contracts'
...
const pending = ref(false), error = ref(''), reportOpen = ref(false), deleteOpen = ref(false), reason = ref('内容不准确'), description = ref(''), expanded = ref(false), overflowed = ref(false), exportOpen = ref(false)
...
const exportPost = async (format: CommunityExportFormat) => { try { const result = await communityApi.exportPost(props.post.id, format); download(new Blob([result.content], { type: result.mimeType }), `${result.filename}.${result.extension}`); exportOpen.value = false } catch (cause) { error.value = cause instanceof Error ? cause.message : '导出失败' } }
const addToPortfolio = () => act(async () => { await request(`/portfolio/convert/${encodeURIComponent(props.post.id)}`, { method: 'POST' }) })
