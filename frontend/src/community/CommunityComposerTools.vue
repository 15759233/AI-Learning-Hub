...
<template>
  <div class="composer-tool-panel">
    ...
    <template v-else-if="panel === 'binding'">
      ...
      <div class="community-topic-list"><button v-for="(binding, index) in form.bindings" :key="`${binding.type}:${binding.id}`" type="button" class="community-chip" @click="form.bindings.splice(index, 1)">{{ bindingTitles[`${binding.type}:${binding.id}`] || binding.id }} ×</button></div>
    </template>
    <template v-else>
