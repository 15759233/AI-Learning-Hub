...
<template>
  <form class="dialog-form composer-form" @submit.prevent="save()" @keydown="keydown" @paste="paste" @dragover.prevent @drop="drop">
    ...
    <label v-if="form.visibility === 'public' && !form.contribution"><input v-model="form.portalConsent" type="checkbox" :disabled="saving" />允许未登录门户展示本内容摘要、署名及公开主页简介（可随时编辑撤回）</label>
