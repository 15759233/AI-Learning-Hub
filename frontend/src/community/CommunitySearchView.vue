...
const selectType = (value: CommunitySearchType) => router.push({ path: '/community/search', query: { q: keyword.value, type: value } })
...
<template><section><header class="community-page-heading"><div><h1>{{ route.query.bindingId ? '相关学习讨论' : '搜索学习社区' }}</h1><p>让内容、同行者和学习资源连在一起。</p></div></header><form class="community-search" @submit.prevent="selectType(type)"><input v-model="keyword" placeholder="搜索动态、用户、话题与学习内容" aria-label="搜索关键词" maxlength="120" /><button class="button primary small">搜索</button></form><nav v-if="!route.query.bindingId" class="community-feed-tabs search-tabs"><button v-for="(label, value) in labels" :key="value" :aria-selected="type === value" @click="selectType(value)">{{ label }}</button></nav><CommunitySkeleton v-if="loading && !result" /><p v-if="error" class="community-error" role="alert">{{ error }} <button @click="load()">重新搜索</button></p><template v-if="result"><section v-if="result.posts.length"><header class="search-section-heading"><h2>动态</h2><button v-if="type === 'all'" class="text-link" @click="selectType('posts')">查看全部动态</button></header><CommunityPostCard v-for="post in result.posts" :key="post.id" :post="post" @changed="load()" @hidden="load()" /></section><section v-for="key in (['users', 'topics'] as const)" :key="key"><header v-if="result[key].length" class="search-section-heading"><h2>{{ labels[key] }}</h2><button v-if="type === 'all'" class="text-link" @click="selectType(key)">查看全部{{ labels[key] }}</button></header><RouterLink v-for="row in result[key]" :key="row.id" class="community-binding" :to="'username' in row ? `/community/user/${row.username}` : `/community/topic/${row.slug}`">{{ 'displayName' in row ? row.displayName : `# ${row.name}` }}</RouterLink></section><section v-for="kind in kinds" :key="kind"><header v-if="result[kind].length" class="search-section-heading"><h2>{{ labels[kind] }}</h2><button v-if="type === 'all'" class="text-link" @click="selectType(kind)">查看全部{{ labels[kind] }}</button></header><RouterLink v-for="row in result[kind]" :key="row.id" class="community-binding search-learning-result" :to="link(kind, row.id)" @click="record(kind, row.id)"><strong>{{ row.title }}</strong><p>{{ row.summary }}</p><small>进入{{ labels[kind] }} ↗</small></RouterLink></section><button v-if="result.nextCursor" class="button secondary" :disabled="loading" @click="load(true)">加载更多</button><div v-if="empty && !loading" class="community-empty"><h2>{{ keyword || route.query.bindingId ? '没有找到相关结果' : '从一个关键词开始' }}</h2><p>试试课程名、学习方向或用户名。</p><RouterLink class="button secondary" to="/community">返回社区发现</RouterLink></div></template></section></template>
