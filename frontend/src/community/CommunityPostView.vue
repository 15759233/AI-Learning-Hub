...
<template><section><header class="community-page-heading"><RouterLink :to="store.lastFeedLocation">← 返回社区</RouterLink><h1>学习讨论</h1></header><p v-if="error" class="community-error" role="alert">{{ error }} <button @click="load">重试</button></p><CommunityPostCard v-if="post" :post="post" detail @changed="load" @hidden="post = null" />
  <section v-if="post && post.status !== 'draft'" class="community-discussion"><h2>{{ post.type === 'question' ? '回答与交流' : '学习讨论' }} · {{ post.stats.comments }}</h2><div v-if="post.question?.acceptedCommentId" class="accepted-notice">✓ 已采纳回答：{{ comments.find((c) => c.id === post?.question?.acceptedCommentId)?.body }}</div>
    <article v-for="comment in comments" :key="comment.id" class="community-comment" :class="{ reply: comment.parentId, accepted: comment.accepted }"><header><span class="avatar">{{ comment.author.displayName[0] }}</span><strong>{{ comment.author.displayName }}</strong><small>{{ badgeLabels[comment.author.verifiedType] }}</small><span v-if="comment.accepted" class="community-badge">已采纳</span></header><p v-if="comment.deleted" class="muted">{{ comment.body }}</p><CommunityBlocks v-else :blocks="comment.contentBlocks" /><footer v-if="!comment.deleted"><button v-if="!comment.parentId" class="text-link" @click="replyTo = comment; editId = ''; body = ''">回复</button><button class="text-link" @click="act(() => communityApi.commentLike(comment.id, !comment.liked))">{{ comment.liked ? '已赞' : '赞' }} {{ comment.likes || '' }}</button><button v-if="post.type === 'question' && post.author.id === auth.user?.id && !comment.accepted" class="text-link" @click="act(() => communityApi.accept(post!.id, comment.id))">采纳回答</button><template v-if="comment.author.id === auth.user?.id"><button class="text-link" @click="edit(comment)">编辑</button><button class="text-link" @click="act(() => communityApi.removeComment(comment.id))">删除</button></template><button v-else class="text-link" @click="act(() => communityApi.report(comment.id, '不当评论', '请求人工复核讨论内容', true))">举报</button></footer></article>
