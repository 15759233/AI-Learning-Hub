...
const load = async () => { try { post.value = await communityApi.post(props.postId || String(route.params.postId)); comments.value = await communityApi.comments(post.value.id); error.value = ''; await nextTick(); if (route.hash) document.getElementById(route.hash.slice(1))?.scrollIntoView({ block: 'center' }) } catch (cause) { post.value = null; error.value = cause instanceof Error ? cause.message : '动态读取失败' } }
const act = async (action: () => Promise<unknown>) => { if (!requireWrite()) return; pending.value = true; try { await action(); await load() } catch (cause) { error.value = cause instanceof Error ? cause.message : '操作失败' } finally { pending.value = false } }
const edit = (comment: CommunityCommentDto) => { if (!requireWrite()) return; editId.value = comment.id; body.value = comment.contentBlocks.filter((block) => block.type === 'paragraph').map((block) => block.text).join('\n'); replyTo.value = null }
const reply = (comment: CommunityCommentDto) => { if (!requireWrite()) return; replyTo.value = comment; editId.value = ''; body.value = '' }
...
watch(() => props.postId || route.fullPath, load, { immediate: true })
</script>
