...
export const useCommunityDraft = defineStore('community-draft', () => {
  ...
  const preview = ref(false), saving = ref(false), bindingLoading = ref(false), error = ref(''), savedAt = ref(''), closePrompt = ref(false), dirty = ref(false), draftId = ref<string>()
  ...
  watch(() => store.composerOpen, async (open) => {
    ...
    if (!store.editingId && !value.contentBlocks.length && !value.bindings.length) {
      try { const local = JSON.parse(localStorage.getItem(key()) || 'null') as (CommunityDraftDto & { editingId?: string }) | null; if (local?.input) { value = local.input; draftId.value = local.id || undefined; store.editingId = local.editingId; savedAt.value = '已恢复本地草稿' } } catch { error.value = '本地草稿格式异常，可从草稿箱恢复' }
    }
    hydrate(value)
    try {
      if (!topics.value.length) { const rows = await communityApi.topics(); if (epoch !== store.epoch || owner !== auth.user?.id) return; topics.value = rows }
  ...
  const localSave = () => { localStorage.setItem(key(), JSON.stringify({ id: draftId.value || '', editingId: store.editingId, input: input(), updatedAt: new Date().toISOString() })); savedAt.value = '已保存到当前浏览器' }
  const save = (asDraft = false): Promise<boolean> => {
    ...
    const operation = Promise.resolve().then(async () => {
      ...
      try {
        ...
        else { clearTimeout(timer); clearTimeout(remoteTimer); localStorage.removeItem(key()); dirty.value = false; draftId.value = undefined; store.published(post) }
        return true
  ...
  watch([form, body, code, quote, language, images], () => {
    ...
    dirty.value = true; clearTimeout(timer); clearTimeout(remoteTimer)
    const owner = auth.user?.id, epoch = store.epoch
  ...
  watch([() => auth.user?.id, () => store.epoch], () => {
    ...
    body.value = ''; code.value = ''; quote.value = ''; images.value = []; topics.value = []; bindingTitles.value = {}; bindingLoading.value = false; draftId.value = undefined; dirty.value = false; saving.value = false; error.value = ''; closePrompt.value = false; pending = null
    form.value = { type: 'general', title: '', contentBlocks: [], bindings: [], topicIds: [], visibility: 'public', status: 'published' }
    queueMicrotask(() => { hydrating = false })
  }, { flush: 'sync' })
  const close = () => { if (dirty.value) closePrompt.value = true; else store.composerOpen = false }
  const discard = () => { clearTimeout(timer); clearTimeout(remoteTimer); localStorage.removeItem(key()); dirty.value = false; closePrompt.value = false; store.composerOpen = false }
  const saveAndClose = async () => { if (await save(true)) { closePrompt.value = false; store.composerOpen = false } }
  return { form, body, code, language, quote, images, topics, bindingType, bindingId, bindingSearch, bindingTitles, bindingOptions, source, preview, saving, bindingLoading, error, savedAt, closePrompt, dirty, draftId, blocks, advanced, loadOptions, addBinding, upload, uploadFiles, save, restore, close, discard, saveAndClose }
})
