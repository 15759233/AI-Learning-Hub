...
const saveCollection = async () => {
  ...
  try {
    ...
    else await resourceHubApi.createCollection(input)
    collectionOpen.value = false
    notice.value = collectionForm.id ? '合集已更新' : '合集已创建'
    await load()
...
<template>
  <section class="page-container resource-studio-page">
    <header>
      <div><span>个人工作台</span><h1>我的资源创作</h1><p>管理已发布作品、草稿、处理中的视频和学习合集。</p></div>
      ...
      <section>
        ...
        <div class="resource-studio-collections">
          <article v-for="item in collections" :key="item.id">
            <RouterLink :to="`/resources/collections/${item.systemKind === 'watch_later' ? 'watch-later' : item.id}`"><AppIcon :name="item.systemKind === 'watch_later' ? 'bookmark' : 'folder'" /><span><strong>{{ item.name }}</strong><small>{{ item.itemCount }} 项 · {{ item.visibility === 'private' ? '私有' : '社区可见' }}</small></span></RouterLink>
