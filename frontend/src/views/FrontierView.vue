...
<template>
  <div class="page-container frontier-page">
    <PageHero eyebrow="阅读与发现" title="AI 前沿" description="用清晰、可信的技术解读，理解 AI 世界正在发生什么。" visual-key="frontierHeroAssetId" />
    ...
    <div class="editorial-layout">
      <section>
        ...
        <div class="editorial-list"><RouterLink v-for="article in filtered.slice(0, visible)" :key="article.id" class="editorial-item" :to="{ path: '/frontier', query: { article: article.id } }"><CategoryCover :title="article.title" :media="article" :variant="article.coverVariant" :icon="article.icon" /><div><span class="tag">{{ article.category }}</span><h3>{{ article.title }}</h3><p>{{ article.summary }}</p><small>{{ article.publishedAt || '发布时间 —' }} · {{ article.readMinutes === undefined ? '阅读时长 —' : `${article.readMinutes} 分钟阅读` }}{{ dataMode === 'mock' ? ' · 演示资讯' : '' }}</small></div></RouterLink></div>
