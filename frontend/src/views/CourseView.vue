...
const course = computed(() => courses.find((item) => item.id === route.params.courseId) || courses[0])
...
const noteDraft = ref(store.notes)
...
const saveNote = () => {
  store.saveNote(noteDraft.value)
...
<template>
  <div class="page-container">
    <section class="course-hero">
      ...
      <aside class="hero-progress"><ProgressBar :value="store.courseProgress[course.id] || course.progress || 0" label="学习进度" /><strong>已学 {{ currentLesson }} / 18 课时</strong><button class="button primary full-width" type="button" @click="store.completeCourseStep(course.id)">完成本节</button><button class="button secondary full-width" type="button" @click="store.toggleFavorite(course.id)">{{ store.favorites.includes(course.id) ? '已收藏' : '收藏课程' }}</button></aside>
    </section>
    <div class="learning-layout">
      ...
      <article class="lesson-content">
        ...
        <section class="code-card"><div><span>Python</span><button type="button" @click="copyCode">{{ copied ? '已复制' : '复制代码' }}</button></div><pre><code>{{ code }}</code></pre></section>
      ...
      <aside class="lesson-aside sticky">
        <section><div class="panel-title"><strong>我的笔记</strong><button type="button" @click="noteOpen = true">编辑</button></div><p>{{ store.notes || '还没有笔记，记录一个关键想法吧。' }}</p></section>
    ...
    <section><div class="section-heading"><h2>相关推荐课程</h2></div><div class="four-grid"><CourseCard v-for="item in courses.slice(1, 5)" :key="item.id" :course="item" compact /></div></section>
  </div>
  <dialog :open="noteOpen" class="search-dialog" @close="noteOpen = false"><form class="dialog-card" @submit.prevent="saveNote"><div class="dialog-title"><strong>本节学习笔记</strong><button class="icon-button" type="button" @click="noteOpen = false">×</button></div><textarea v-model="noteDraft" rows="7" placeholder="记录关键概念和待解决的问题…" /><button class="button primary" type="submit">保存到本地</button></form></dialog>
</template>
