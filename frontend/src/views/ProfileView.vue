...
const favoriteItems = computed(() => {
  const all = [
    ...courses.value.map((item) => ({ id: item.id, title: item.title, type: '课程', favoriteType: 'course' as const, date: '' })),
    ...labs.value.map((item) => ({ id: item.id, title: item.title, type: '实验', favoriteType: 'lab' as const, date: '' })),
    ...resources.value.map((item) => ({ id: item.id, title: item.title, type: '资源', favoriteType: 'resource' as const, date: item.updatedAt })),
    ...articles.value.map((item) => ({ id: item.id, title: item.title, type: '文章', favoriteType: 'article' as const, date: item.publishedAt })),
...
<template>
  <div class="page-container profile-page">
    ...
    <section><div class="section-heading"><h2>学习总览</h2><RouterLink to="/assessments">学习数据详情 →</RouterLink></div><p v-if="!accountDataReady" class="notice">{{ accountDataMessage }}</p><div class="stat-row six"><article v-for="[label, value] in overviewRows" :key="label"><strong>{{ value }}</strong><span>{{ label }}</span></article></div></section>
    ...
    <section><div class="section-heading"><h2>我的徽章墙</h2><button v-if="dataMode === 'mock'" class="text-link" type="button" @click="badgeOpen = '全部徽章'">查看全部（86）→</button></div><div v-if="dataMode === 'mock'" class="badge-wall"><button v-for="(badge, index) in [...badges, '更多成就']" :key="badge" type="button" :class="{ locked: index >= 6 }" @click="badgeOpen = badge"><span>{{ index === 6 ? '◇' : ['◆', '⬢', '✦', '✹', '⌘', '●'][index] }}</span><strong>{{ badge }}</strong><small>{{ index >= 6 ? '保持学习解锁' : '已获得' }}</small></button></div><p v-else-if="accountDataReady" class="notice">服务端已记录 {{ store.serverGrowth?.achievements ?? 0 }} 枚成就、{{ store.serverGrowth?.certificates ?? 0 }} 份证书。</p><p v-else class="notice">{{ accountDataMessage }}</p></section>
    <div class="profile-two-column">
      <section><div class="section-heading"><h2>最近学习课程</h2><RouterLink to="/topics">全部课程 →</RouterLink></div><p v-if="!accountDataReady" class="notice">{{ accountDataMessage }}</p><div v-else-if="recentCourses.length" class="record-list"><article v-for="course in recentCourses" :key="course.id"><div><strong>{{ course.title }}</strong><small>{{ course.hours }} 小时 · {{ dataMode === 'api' ? '账号学习记录' : '本地学习记录' }}</small><ProgressBar :value="store.courseProgress[course.id] || 0" /></div><RouterLink class="button primary small" :to="`/courses/${course.id}`">继续学习</RouterLink></article></div><p v-else>暂无最近学习课程。</p></section>
      <section><div class="section-heading"><h2>实践记录</h2><RouterLink to="/labs">全部实验 →</RouterLink></div><p v-if="!accountDataReady" class="notice">{{ accountDataMessage }}</p><div v-else-if="recentLabs.length" class="record-list"><article v-for="lab in recentLabs" :key="lab.id"><div><strong>{{ lab.title }}</strong><small>完成度 {{ store.labProgress[lab.id] || 0 }}% · {{ dataMode === 'api' ? '账号实践记录' : '本地演示记录' }}</small><ProgressBar :value="store.labProgress[lab.id] || 0" /></div><RouterLink class="button primary small" :to="`/labs/${lab.id}`">进入实验</RouterLink></article></div><p v-else>暂无实践记录。</p></section>
    </div>
    <div class="profile-two-column">
      <section class="favorites-panel"><div class="section-heading"><h2>我的收藏</h2></div><p v-if="!accountDataReady" class="notice">{{ accountDataMessage }}</p><template v-else><div class="compact-tabs"><button v-for="tab in ['全部', '课程', '实验', '资源', '文章']" :key="tab" type="button" :class="{ active: favoriteTab === tab }" @click="favoriteTab = tab">{{ tab }}</button></div><div v-if="favoriteItems.length" class="favorite-list"><div v-for="item in favoriteItems" :key="`${item.favoriteType}-${item.id}`"><span class="tag">{{ item.type }}</span><strong>{{ item.title }}</strong><small>收藏于 {{ item.date }}</small><button type="button" @click="store.toggleFavorite(item.favoriteType, item.id)">取消收藏</button></div></div><div v-else class="inline-empty small-empty"><p>暂无收藏内容。</p><RouterLink to="/topics">去发现课程 →</RouterLink></div></template></section>
      <section class="plan-card"><div class="section-heading"><h2>学习计划</h2><button v-if="accountDataReady" class="text-link" type="button" @click="planOpen = true">创建计划 →</button></div><p v-if="!accountDataReady" class="notice">{{ accountDataMessage }}</p><div v-else-if="store.plans.length" class="plan-list"><article v-for="plan in store.plans" :key="plan.id"><span class="tag green">{{ plan.status }}</span><h3>{{ plan.name }}</h3><p>目标日期：{{ plan.targetDate }}</p><button class="button secondary small" type="button" @click="store.togglePlan(plan.id)">{{ plan.status === '进行中' ? '标记完成' : '恢复进行中' }}</button></article></div><div v-else class="inline-empty small-empty"><p>暂无学习计划。</p></div></section>
    </div>
