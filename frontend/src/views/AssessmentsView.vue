...
<template>
  <div class="page-container assessment-page">
    ...
    <div class="assessment-grid">
      <section class="exam-card"><span class="eyebrow">全真模拟测评</span><h2>AI 综合能力测评</h2><p>60 道题 · 90 分钟 · 综合难度</p><div class="exam-stats"><span><strong>3</strong>历史测评</span><span><strong>82%</strong>最佳正确率</span><span><strong>A</strong>当前等级</span></div><button class="button primary" type="button" @click="quizBridge.startAssessment('full-ai')">开始模拟测评</button><button class="text-link" type="button" @click="quizBridge.openReport('full-ai')">历史记录 →</button></section>
  ...
  <dialog :open="ruleOpen" class="search-dialog" @close="ruleOpen = false"><div class="dialog-card"><div class="dialog-title"><strong>挑战规则</strong><button class="icon-button" type="button" @click="ruleOpen = false">×</button></div><ol><li>答题入口由《题盒》统一提供。</li><li>本页不保存题库、计时或判分逻辑。</li><li>演示积分不会写入服务器。</li></ol></div></dialog>
  <dialog :open="wrongOpen" class="search-dialog" @close="wrongOpen = false"><div class="dialog-card"><div class="dialog-title"><strong>错题解析</strong><button class="icon-button" type="button" @click="wrongOpen = false">×</button></div><p>注意力机制通过相关性权重聚合不同位置的信息，从而建立上下文联系。</p><button class="button primary" type="button" @click="quizBridge.startPractice('attention')">通过《题盒》继续练习</button></div></dialog>
</template>
