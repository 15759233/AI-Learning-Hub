...
<template>
  <div class="community-shell" :class="{ 'sidebar-collapsed': collapsed }">
    ...
    <aside class="community-sidebar">
      ...
      <nav aria-label="学习社区导航"><RouterLink v-for="item in communityNavigation" :key="item.path" :to="item.path" :title="item.label"><AppIcon :name="item.icon" :size="21" /><span class="nav-label">{{ item.label }}</span><b v-if="item.path === '/notifications' && store.unread" class="notification-count">{{ store.unread }}</b></RouterLink></nav>
      ...
      <div class="community-account"><span class="avatar">{{ auth.user?.displayName.slice(0, 1) }}</span><div class="nav-label"><strong>{{ auth.user?.displayName }}</strong><small>{{ auth.dataMode === 'mock' ? '显式演示模式' : '统一学习账号' }}</small><RouterLink :to="`/community/user/${auth.user?.id}`">个人主页与设置</RouterLink></div><button class="icon-button" type="button" aria-label="退出登录" title="退出登录" @click="logout"><AppIcon name="arrow-left" :size="18" /></button></div>
      <RouterLink class="text-link nav-label portal-link" to="/welcome">查看品牌门户</RouterLink>
    </aside>
    <header class="community-mobile-header"><RouterLink class="brand" to="/community"><span class="brand-mark">A</span><strong>AI MAKER CAMPUS</strong></RouterLink><button class="icon-button" aria-label="更多功能" @click="menuOpen = true"><AppIcon name="menu" /></button><button class="button primary small" @click="store.openComposer()">发布</button></header>
    <main id="main-content" class="community-main"><slot /></main>
    <CommunityRightRail />
    <nav class="community-bottom-nav" aria-label="移动主导航"><RouterLink v-for="item in [communityNavigation[0], communityNavigation[1], communityNavigation[2], communityNavigation[8], communityNavigation[6]]" :key="item.path" :to="item.path"><AppIcon :name="item.icon" :size="21" /><span>{{ item.label.replace('首页', '').replace('主题', '').replace('项目', '').replace('消息', '').replace('成长', '') }}</span></RouterLink></nav>
