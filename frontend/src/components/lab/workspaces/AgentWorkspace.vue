...
<template>
  <div ref="host" class="workspace-type">
    <div class="flow-toolbar"><strong>Agent 工作流设计</strong><span /><button type="button" @click="autoLayout">自动布局</button><button type="button" @click="zoomOut()">缩小</button><button type="button" @click="zoomIn()">放大</button><button type="button" @click="fitView({ padding: 0.16 })">适应画布</button><button type="button" @click="toggleFullscreen">全屏</button></div>
