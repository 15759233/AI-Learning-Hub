...
const setup = (targetType = 'post') => {
  ...
  const controller = new CommunityAdminController(prisma as never, {} as never, {} as never, {} as never, {} as never, {} as never, {} as never, visibility as never, {} as never)
...
describe('内容复核后台读取边界', () => {
  ...
  it('帖子详情只读取匹配复核修订的文字，访问日志仅记录对象ID', async () => {
    ...
    expect(prisma.communityPost.findFirst).toHaveBeenCalledWith({ where: { id: row.targetId, revision: 3 }, include: { contribution: true } })
...
describe('后台隐藏后恢复的增量检测', () => {
  it.each((['post', 'comment'] as const).flatMap((target) => (['allow', 'warn', 'review', 'reject'] as const).map((action) => ({ target, action }))))('$target恢复重新检测并执行$action', async ({ target, action }) => {
    ...
    const tx = {
      ...
      communityModerationAction: { create: vi.fn() }, activityEvent: { create: vi.fn() }, contentReview: { updateMany: vi.fn(), create: vi.fn() },
    ...
    const controller = new CommunityAdminController(prisma as never, {} as never, {} as never, notifications as never, {} as never, {} as never, {} as never, { adminWhere: vi.fn(async () => ({ status: { not: 'draft' } })) } as never, detection)
    const user = { id: 'synthetic-reviewer', permissions: ['community.moderate'] } as never
    await controller.moderate(user, target, row.id, { action: 'hide', reason: '隐藏合成测试内容' })
