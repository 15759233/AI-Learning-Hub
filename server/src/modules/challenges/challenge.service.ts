...
const dataFields = ['cover', 'startAt', 'endAt', 'leaderboardEnabled', 'integration']
...
export class ChallengeService {
  ...
  async update(id: string, input: UpdateChallengeDto, actorId: string) {
    ...
    const current = await this.prisma.challenge.findUnique({ where: { id } })
    if (!current) throw new NotFoundException('挑战不存在')
    const data = { ...this.support.data(current.payload), ...this.support.pick(input, dataFields) }
    const item = await this.prisma.challenge.update({
      where: { id },
      data: {
        ...this.support.pick(input, ['title', 'summary', 'sortOrder', 'challengeType', 'targetScore', 'rewardPoints']),
        payload: this.support.sanitize(data),
        version: { increment: 1 },
      },
    })
  ...
  async linkQuestionBank(id: string, questionBankId: string) {
    await this.ensureDraft(id)
    await this.prisma.challenge.update({ where: { id }, data: { questionBankId } })
    await this.refreshDraft(id)
  ...
  async linkPaper(id: string, paperId: string) {
    await this.ensureDraft(id)
    await this.prisma.challenge.update({ where: { id }, data: { paperId } })
    await this.refreshDraft(id)
    return this.detail(id)
  }

  private async ensureDraft(challengeId: string) {
    ...
    const version = await this.prisma.challengeVersion.create({
      data: {
        challengeId,
    ...
    await this.prisma.challenge.update({ where: { id: challengeId }, data: { currentDraftVersionId: version.id } })
    return version.id
  }

  private async refreshDraft(challengeId: string) {
    const challenge = await this.prisma.challenge.findUnique({ where: { id: challengeId }, include: { rules: { orderBy: { ruleKey: 'asc' } } } })
    if (!challenge?.currentDraftVersionId) throw new NotFoundException('挑战草稿不存在')
    await this.prisma.challengeVersion.update({
